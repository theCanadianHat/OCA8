...
- [X] B. An immutable object cannot be modified.
- [X] C. An immutable object can be garbage collected.
- [ ] D. An immutable object cannot be garbage collected.
- [X] E. `String` is immutable.
...
- [X] C. `puzzle.append("vaJ$").delete(0, 3).deleteCharAt(pzuzzle.length() -1);`
...
- [X] C. `String beans[] = new beans[6];`
- [ ] D. `java.util.Date[] dates[] = new java.util.Date[2][];`
- [X] E. `int[][] types = new int[];`
- [X] F. `int[][] java = new int[][];`
...
- [X] A. An array has a fixed size.
- [ ] B. An `ArrayList` has a fixed size.
- [X] C. An array allows multiple demensions.
- [X] D. An array is ordered.
- [X] E. An `ArrayList` is ordered.
...
- [X] B. Two `ArrayList`s with the same content are equal.
- [X] C. If you call `remove(0)` using an empty `ArrayList` object, it will compile successfully.
...
```
- [X] A. The code compiles.
- [X] B. The code throws a runtime exception.
- [ ] C. Exactly one of the add statements uses autoboxing.
- [X] D. Exactly two of the add statements use autoboxing.
...
- [X] D. `LocalDate.of(2014, 6, 21);`
- [ ] E. `LocalDate.of(2014, Calendar.JUNE, 21);`
- [X] F. `LocalDate.of(2014, Month.JUNE, 21);`
...
- [ ] D. Another date.
- [ ] E. The code does not compile.
- [X] F. A runtime exception is thrown.
